...
public:
  ...
  virtual bool scatter(const ray &r_in, const hit_record &rec,
    ...
    double refaction_ratio = rec.front_face ? (1.0 / ir) : ir;
    ...
    bool cannot_refract = refaction_ratio * sin_theta > 1.0;

    vec3 direction;

    if (cannot_refract ||
        reflectance(cos_theta, refaction_ratio) > random_double())
      direction = reflect(unit_direction, rec.normal);
    else
      direction = refract(unit_direction, rec.normal, refaction_ratio);
